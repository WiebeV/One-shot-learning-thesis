...
class MiniImageNet(Dataset):
    def __init__(self, subset):
        ...
        if subset not in ('datasets/in32_all', 'datasets/test'):
            raise(ValueError, 'subset must be one of (background, evaluation)')
        self.subset = subset
        ...
        self.transform = transforms.Compose([
            transforms.Grayscale(num_output_channels = 3), #added code
            transforms.CenterCrop(224),
            transforms.Resize(84), #default 84
            transforms.ToTensor(),
    ...
    def index_subset(subset):
        ...
        for root, folders, files in os.walk(DATA_PATH + '/{}/'.format(subset)): #use for pretraining
