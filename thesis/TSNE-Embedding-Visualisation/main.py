...
def populate_img_arr_modified(images_paths, size=(100, 100), should_preprocess=False):
    ...
    """
    arr = []
    x_ = np.zeros((1, 128, 128, 3))
    y_ = np.zeros((1, 128, 128, 3))
    ...
    for i, img_path in enumerate(images_paths):
        ...
        # x = x.astype('float32')
        x = x / 255
        arr.append([x_,y_, np.expand_dims(x,axis=0)])
        if i>8:
            break
    #arr = np.array(arr)
...
@click.option('--model_input_size', default=128, help='Size of inputs to model')
def main(data, name, sprite_size, tensor_name, sprite_name, model_input_size):
    ...
    model = load_model('/home/deeplearning/ai-transferlearning/thesis/siamese_own_implementation/trained_models/Klaar IN32-1 deel 1 wobbler2x, leaflet2x, display2x, poster4x/' + trained_weights, compile=False)
    #model = InceptionV3(include_top=False, pooling='avg')

    img_arr = populate_img_arr_modified(images_paths, size=(model_input_size, model_input_size), should_preprocess=False)
    # x_ = np.zeros((1, 128, 128, 3))
    # y_ = np.zeros((1, 128, 128, 3))
    preds = model.predict(img_arr[0], batch_size=64)#[:,2]
